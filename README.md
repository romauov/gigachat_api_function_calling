...
```
UPD.
16 апреля 2024 Сбер открыл доступ к функционалу ещё для двух своих моделей, и теперь он доступен для `GigaChat-preview`, `GigaChat-Plus-preview` или `GigaChat-Pro-preview`.
Запросы с функциями работают не очень стабильно, поэтому, если видите, что ответ долго не приходит, не стесняйтесь обновлять запрос.
```
